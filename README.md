...
- repo: local
  hooks:
    - id: actionlint
      ...
      #additional_dependencies: [actionlint-py==1.7.5.18]
      # safer, but pre-commit autoupdate will not work
      # note: the pip versioning scheme is different from actionlint binary: not "v1.7.5" but "1.7.5.18" (last number is build system version)
      entry: actionlint
