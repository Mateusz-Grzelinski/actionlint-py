...
- repo: local
  hooks:
    - id: actionlint
      ...
      #additional_dependencies: [actionlint-py==1.7.12.24.dev.3]
      # safer, but pre-commit autoupdate will not work
      # note: the pip versioning scheme is different from actionlint binary: not "v1.7.12" but "1.7.12.24.dev.3" (last number is build system version)
      entry: actionlint
