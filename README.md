...
- repo: local
  hooks:
    - id: actionlint
      ...
      #additional_dependencies: [actionlint-py==1.6.25.9.dev.2]
      # safer, but pre-commit autoupdate will not work
      # note: the versioning scheme is different: not "v1.6.25" but "1.6.25.9.dev.2" (last number is build system version)
      entry: actionlint
